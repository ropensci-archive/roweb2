...
[`drake`](https://github.com/ropensci/drake) watches your session and environment for dependencies. This behavior frees [`drake`](https://github.com/ropensci/drake) to fully focus on R, which enhances interactivity, flexibility, and independence from cumbersome configuration files. However, a fully reproducible use of [`drake`](https://github.com/ropensci/drake) requires care. Version 7.0.0 implements safeguards to nudge users in the right direction.

### Interactive sessions

A serious [`drake`](https://github.com/ropensci/drake) workflow should be consistent and reliable, ideally with the help of a [master R script](https://github.com/wlandau/drake-examples/blob/master/gsp/make.R). This script should begin in a fresh R session, load your packages and functions in a [dependable manner](https://github.com/wlandau/drake-examples/blob/d9417547a05aec416afbbda913eaf2d44a552d5b/gsp/make.R#L4-L6), and then run [`make()`](https://ropensci.github.io/drake/reference/make.html). [Batch mode](https://blog.revolutionanalytics.com/2009/06/batch-mode-in-r-a-primer.html), especially within a [container](https://www.docker.com), is particularly helpful.

[Interactive sessions](https://stat.ethz.ch/R-manual/R-devel/library/base/html/interactive.html) are still useful, but they easily grow stale. Targets can falsely invalidate if you accidentally change a function or data object in your environment. So in [interactive mode](https://stat.ethz.ch/R-manual/R-devel/library/base/html/interactive.html), [`make()`](https://ropensci.github.io/drake/reference/make.html) now pauses to protect you from the pitfalls.
...
You can disable the menu with `make(force = TRUE)` or `options(drake_force_interactive = TRUE)`, the latter of which you might include in your local [`.Rprofile` file](https://csgillespie.github.io/efficientR/3-3-r-startup.html#rprofile).
...
The error above comes from the call to `data(mtcars)`. Without guardrails, the very act of building `x` changes `x`'s dependencies.
