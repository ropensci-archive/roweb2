...
For parallel deployment to a computing cluster ([SLURM](https://slurm.schedmd.com/), [TORQUE](http://www.adaptivecomputing.com/products/open-source/torque/), [SGE](http://www.univa.com/products/), etc.) `drake` turns to packages [`future`](https://github.com/HenrikBengtsson/future), [`batchtools`](https://github.com/mllg/batchtools), and [`future.batchtools`](https://github.com/HenrikBengtsson/future.batchtools). First, create a [`batchtools` template file](https://github.com/mllg/batchtools/tree/master/inst/templates) to declare your resource requirements and [environment modules](http://modules.sourceforge.net/). There are built-in example files in [`drake`](https://github.com/ropensci/drake), but you will likely need to tweak your own by hand.
...
The `parallelism` argument of `make()` controls not only where to deploy the workers, but also how to schedule them. The following table categorizes the 7 possible options.
...
The first parallel algorithm to enter [`drake`](https://github.com/ropensci/drake) was staged scheduling. It is easier to implement than the other two, but it forces the workers run in lockstep. In other words, all the workers have to finish their current targets before any worker can move on. The following animation illustrates the concept.
...
If the time limits of your cluster are too strict, consider transient scheduling instead, another new arrival. Here, `make()` starts a brand new worker for each individual target. See the following video.
...
Functions `predict_runtime()` and `predict_load_balancing()` can help. Let's revisit the [`mtcars` example](https://ropensci.github.io/drake/articles/example-mtcars.html).
...
2. Assume each non-file target (black circle) takes 2 hours to build.
3. Rest assured that everything else is super quick.

If you declare the runtime assumptions with the `known_times` argument and cycle over a reasonable range of `jobs`, `predict_runtime()` paints a clear picture.

<div style = "text-align: center">
<img src="https://cdn.rawgit.com/ropensci/drake/7a9caec7/images/times.png"
