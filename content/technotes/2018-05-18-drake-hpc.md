...
`jobs = 4` is a solid choice. Any fewer would slow us down, and to shave off another 2 hours, it would take double the `jobs` and the hardware to back it up. Your choice of `jobs` for `make()` ultimately depends on the runtime you can tolerate and the computing resources at your disposal.
