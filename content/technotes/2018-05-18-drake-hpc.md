...
Persistent and transient scheduling are new additions to [`drake`](https://github.com/ropensci/drake). The original algorithm is staged scheduling, in which there is no master process and the workers run in lockstep.
...
Let's plan for non-staged scheduling, assume every formal non-file target (black circle) takes 2 hours to build, and rest assured that everything else is super quick. If you declare the runtime assumptions with the `known_times` argument and cycle over a reasonable range of `jobs`, `predict_runtime()` paints a clear picture.
...
This post is a product of my own personal experiences and opinions and does not necessarily represent the official views of my employer. The embedded [PowToon](https://www.powtoon.com) videos were created and used only as explicitly permitted in the [Terms and Conditions of Use](https://www.powtoon.com/terms-and-conditions/). I make no copyright claim to any of the constituent graphics.
