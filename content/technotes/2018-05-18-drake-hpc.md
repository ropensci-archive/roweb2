...
Functions `predict_runtime()` and `predict_load_balancing()` provide guidance. Let's take the [`mtcars` example](https://ropensci.github.io/drake/articles/example-mtcars.html) and plan for non-staged scheduling. Also, assume every formal non-file target (black circle) takes 2 hours to build and everything else is super quick. If you use the `known_times` argument to declare these assumptions and then cycle over a reasonable range of `jobs`, `predict_runtime()` paints a clear picture.
...
Your choice of `jobs` for `make()` ultimately depends on the runtime you can tolerate and the computing resources at your disposal. If you rely on a 4-core laptop, maybe call `make(jobs = 4)` and let your project run overnight. But armed with several vacant nodes on a powerful cluster, you might choose 14 jobs and then head out for a long lunch.
