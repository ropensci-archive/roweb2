...
document.addEventListener('DOMContentLoaded', () => {
    ...
    themeToggle.addEventListener('click', () => {
        ...
        if(themeStylesheet.href.includes('friendly')){
            themeStylesheet.href = '/css/vim.css';
...
This color also needs to not endanger the contrast between code and code background, so if your code highlighting is "dark mode", yellow highlighting is probably a bad idea: in this post, for the dark mode, we used the "vim" Chroma style but with `#301934` as background color for the highlighted lines.
