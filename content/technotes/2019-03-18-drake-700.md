...
You can disable the menu with `make(force = TRUE)` or `options(drake_force_interactive = TRUE)`, the latter of which could be part of your local [`.Rprofile` file](https://csgillespie.github.io/efficientR/3-3-r-startup.html#rprofile).

### Self-invalidation

Another new safeguard prevents workflows from invalidating themselves.
