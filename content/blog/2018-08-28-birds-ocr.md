...
One can use either `tesseract::ocr` for a  
file path, url, or raw vector to image, or `magick::image_ocr` for a
`magick` object which is quite handy in our pipeline.
