...
birder's series](https://ropensci.org/tags/birder)â€¦ let's explore
...
for language detectionâ€¦ Quite the armory! We'll also sprinkle some
...
for which e.g.Â Jim Hester wrote a [minimal R
wrapper](https://github.com/jimhester/flickrr)â€¦ I don't do *everything*
with R scripts (yet).
...
defined `crop_bird` function that crops and cleans the imageâ€¦
...
that interacts with the BHL API. One can e.g.Â search all books by the
