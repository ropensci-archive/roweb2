...
which researchers manually subset results, but likely never to be
...
tedious and potentially leads to errors, and we see
[Rclean](https://docs.ropensci.org/rclean) as a useful tool for
everyone at all levels of coding ability in R. Here, we'll cover
...
![Example of the plot produced by the code_graph function showing which lines of code produce which variables and which variables are used by other lines of code.](figure/ex-code_graph-1.png)
...
a formal representation of the execution of a computational process
...
the analyses that [Rclean](https://docs.ropensci.org/rclean) conducts
...
objects. [Rclean](https://docs.ropensci.org/rclean) relies on a
library called CodeDepends to gather the prospective provenance for
...
is, we can use `code_graph` function to get a graphical representation
of the prospective provenance generated for
[Rclean](https://docs.ropensci.org/rclean) \ref{fig:prov-graph}.



![Network diagram of the prospective data provenance generated for an example script. Arrows indicate which lines of code (numbered) produced which objects (named).](figure/prov-graph-1.png)
...
algorithms analyze the R script with respect to our results. This is
what empowers the `clean` function, which takes the provenance and
applies a a network search algorithm to determine the pathways leading
...
automatically on comments; however, comments in the original code
...
clearer. This process of comments is an important part of writing
...
[reprex](https://reprex.tidyerse.org) package, which provides a simple
