...
In hydrology, there has been a rapid growth in the number of streamflow data archives made publicly available online by countries such as the UK, USA, Greece, and Canada (although most countries sadly do not yet apply an open policy to their hydrological data). The [Task View on Hydrological Data and Modeling](https://cran.r-project.org/web/views/Hydrology.html) and accompanying blog post [Getting your toes wet in R: Hydrology, meteorology, and more](https://ropensci.org/blog/2019/04/02/hydrology-task-view/) provide an exciting overview of the most up-to-date R packages that are available for downloading, analyzing, and modeling these data. For an overview of the many advantages of using R for hydrological research, see the paper “Using R in Hydrology” [^1]. 
...
There is a wealth of data out there that can be found with careful search queries and many new datasets (like [Uber's micromobility datasets](https://www.geospatialworld.net/blogs/ubers-new-mobility-heatmaps-give-insights-into-micromobility-trends-in-cities/), released on May 6th, 2020) but my starter 5 would be:
