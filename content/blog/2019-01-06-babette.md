...
BEAST2 is a Bayesian phylogenetic tool that helps create a phylogeny from
genetic information. Such genetic information may be a DNA sequence as such:

[](img/blog-images/2019-01-06-babette/alignment.png)

This DNA sequence shows three species, of which species (eloquently 
named) 2 and 3 are more related to each other than species 1, as species
2 and 3 differ only one letter in their DNA sequence. BEAST2 can help
create a phylogeny from this DNA sequence, which looks like this:

[](img/blog-images/2019-01-06-babette/phylogenies.png)

Indeed, these are multiple superimposed phylogenies. These phylogenies
are a representative sample (more likely ones are created more
often) of state space (all possible combinations of phylogenies
and parameter estimates). Such a respresentative sample is called 
a posterior.

I would need thousands of posteriors.
...
BEAUti (I assume the pun is intended) is a beginner-friendly program 
...
Again, I would need thousands of posteriors.

I created `babette` to work with BEAST2 from R.

#### With `babette`

Again, `babette` is a package to work with BEAST2 from R. 
The first step in this workflow is to create a BEAST2 input file,
the second step is to run BEAST2 using that input file.


To create
a BEAST2 input file and run it, one nee
