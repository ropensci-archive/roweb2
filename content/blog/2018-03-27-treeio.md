...
For example, user can parse [BEAST](http://beast2.org/) output using `read.beast` function. All the information inferred by by [BEAST](http://beast2.org/) will be stored in the output object.
...
![hexbin scatter plot: correlation of dN/dS with substitution rate](/img/blog-images/2018-03-27-treeio/hexbin-scatterplot.png)

With is feature, users are able to compare/integrate evolutionary inferences obtained from different software packages/models.

Further readings
----------------

For more details of the importing trees with data, please refer to the [vignette](http://bioconductor.org/packages/devel/bioc/vignettes/treeio/inst/doc/Importer.html). In addition, [treeio](https://bioconductor.org/packages/treeio/) also supports exporing tree data to BEAST Nexus format, which allows software output format conversion and also enables combining external data with tree into a single file.
...
All the data parsed/merged by [treeio](https://bioconductor.org/packages/treeio/) can be converted to tidy data frame using the [tidytree](https://cran.r-project.org/package=tidytree) and can be used to visualize and annotate the tree using [ggtree](https://bioconductor.org/packages/ggtree/).
