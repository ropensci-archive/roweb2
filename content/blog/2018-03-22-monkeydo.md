...
It was clear that if classifying 3 inputs was going to take 10.3 seconds, even classifying my relatively small data was going to take a looong time, like on the order of 3 days, just for the first batch of data 🙈. I updated the function to write each row out to an RDS file after it was classified inside the loop (with an addition along the lines of `write_rds(out[i, ], glue::glue("some_directory/{i}.rds"))`) so that I wouldn't have to rely on the function successfully finishing execution in one run. Still, I didn't like my options.
...
<!--img src="/img/blog-images/2018-04-20-monkeydo/theresgottabeabetterway.gif" style="margin:left 10%"I'm just about at the point where I have to start making myself useful.
...
<!--![](/img/blog-images/2018-04-20-monkeydo/gitflow.jpg)mage source](https://www.slideshare.net/valeriocomo/git-flow-61202423")*
...
##   <chr>                  <int> <chr>                               <list> 
## 1 R2-D2                     96 It is a truth universally acknowle… <data.…
## 2 Ackbar                   180 When Mr. Bilbo Baggins of Bag End … <data.…
## 3 Jabba Desilijic Tiure    175 ""                                  <data.…
## 4 Finn                      NA I'm not an ambiturner. I can't tur… <data.…
## 5 Bail Prestor Organa      191 " "                                 <data.…
```
