...
R packages are widely used in science. Despite this broad usage, the code behind R packages is rarely part of the scientific peer-review process. In the R ecosystem, rOpenSci has been pioneer in developing a peer-review process for R package. The goal of __pkginspector__ is to help that process by providing a means to better understand the internal structure of R packages. It summarizes the relationship between a focal package and the packages it depends on; and reports whether or not functions' interfaces are consistent. If you are reviewing an R package (maybe your own) __pkginspector__ is for you (or at least it will be!).
