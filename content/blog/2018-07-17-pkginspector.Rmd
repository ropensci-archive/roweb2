...
The workhorse function to analyze function calls is `rev_fn_summary()`, which takes a package path and returns a table of information about its functions. We demonstrate here, with a simple package:

```{r, echo=FALSE}
# This approach reduces boilerplate code. pkginspector_example() should be an
# exorted function in pkginspector. For example see ?readr::readr_example()
pkginspector_example <- function() {
  system.file('viridisLite', package = 'pkginspector')
}
```
...
rev_fn_summary(pkginspector_example())
```
...
out <- rev_fn_summary(pkginspector_example())
n <- 1:4
...
```{r, eval = FALSE}
vis_package(pkginspector_example(), physics = FALSE)
```
...
```{r}
rev_args(pkginspector_example())$arg_df
```
