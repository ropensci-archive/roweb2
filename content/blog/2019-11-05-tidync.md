...
In May 2019 version 0.2.0 of [tidync](https://docs.ropensci.org/tidync/) was approved by rOpenSci and accepted to CRAN. Here we provide a quick overview of the typical workflow with some pseudo-code for the main functions in tidync, this is enough to read if you just want to try out the package on your own data.  The tidync package is focussed on *efficient data extraction* for developing your own software, and this somewhat long post takes the time to explain the concepts in detail.  

There is a section about the [NetCDF](https://www.unidata.ucar.edu/software/netcdf/) data model itself. Then there is  a detailed illustration of a raster data set in R including some of challenges faced by R users, followed by sections on how tidync sees coordinates in NetCDF, and how we can *slice a dataset* and control the format of the output. We then discuss some limitations and future work, and then (most importantly) reflect on the rOpenSci process of package review. 
...
*one-dimensional arrays* (i.e. an atomic vector in R of length 1 or more). An array can include coordinate metadata, units, type, and interpretation, and so **attributes** define all of this extra information for dimensions and variables.  The *space* of a variable (the ***grid*** it lives in) is defined by one or more of the dimensions in the file. 
...
We will explore this data set in detail to give an introduction to the tidync summary. The data set is very commonly used in marine research as it includes a very long time series of daily global maps of sea surface temperatures. The resolution is 0.25 degree and the coverage is complete for the entire world's oceans because the temperature data is a blend of remote sensing, direct observations, and models. We call it **OISST** (or *oisst*) because it is the 

> NOAA 1/4° daily *Optimum Interpolation Sea Surface Temperature* (or daily OISST) is an analysis constructed by combining observations from different platforms (satellites, ships, buoys) on a regular global grid. A spatially complete SST map is produced by interpolating to fill in gaps. 

The file we use is a simplified version of the file from 1981-12-31 (the series started in 1981-09-01) and has been reduced in resolution so that it can be stored in an R package. There are four variables in the data **sst** (sea surface temperature, in Celsius), **anom** (the anomaly of sst of this day from the long term mean), **err** (the estimated error standard deviation of sst), and **ice** (the sea ice concentration). The ice concentration acts as a mask, if there is sea ice present in the pixel then the temperature is undefined. 
...
Note that we can use the *internal index* of a variable as well as its name (`anom` is
the variable at the 5 index - a 0-based number -  as shown by `nc_vars()` list just above). 
...
Similarly for dimensions and attributes, but attributes for a variable can only be found by name. 
...
As mentioned, tidync considers time and its metadata a bit dangerous - it is often wrong, often inconsistent, there are time zone issues, sometimes *extra seconds* to account for and so we prefer to leave these interpretations to be validated manually  before automation. 
...
We can choose the middle value of longitude (it lies at index = 90) and plot the `tos` variable as a function of latitude over time. We can easily re-orient our approach to this data set and it works as well with more complicated multi-dimensional sources as well. 
