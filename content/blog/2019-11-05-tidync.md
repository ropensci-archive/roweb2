...
There is a section about the [NetCDF](https://www.unidata.ucar.edu/software/netcdf/) data model itself. Then there is  a detailed illustration of a raster data set in R including some of the challenges faced by R users. This is followed by sections on how tidync sees coordinates in NetCDF, how we can *slice a dataset* and control the format of the output. We then discuss some limitations and future work, and then (most importantly) reflect on the rOpenSci process of package review. 
...
We will explore this data set in detail to give an introduction to the tidync summary. The data set is very commonly used in marine research as it includes a very long time series of daily global maps of sea surface temperatures. The resolution is 0.25 degree and the coverage is complete for the entire world's oceans because the temperature data is a blend of remote sensing and direct observations. We call it **OISST** (or *oisst*) because the 
