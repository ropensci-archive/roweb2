...
Although we had no prior experience working with web data or scrapping APIs for data, we knew several tools existed to interact with APIs in R such as [httr](https://github.com/r-lib/httr) or rOpenSci's [crul](https://github.com/ropensci/crul).
...
Creating these fake responses is not an easy task but fortunately, rOpensci's [vcr](https://github.com/ropensci/vcr) package is exactly suited for this task. vcr  records requests and replays them during ther tests (learn more about it in [the technote about vcr](https://ropensci.org/technotes/2018/05/25/vcr-http-request-cacahing/)). We used it in all our tests as well as for caching the examples shown in the README file. The [companion book](https://ropenscilabs.github.io/http-testing-book/) on HTTP testing helped us dive into the different options suitable to record our requests.
...
Filing issues when you find a bug is an easy but efficient way to give back to the community. [vcr](https://github.com/ropensci/vcr) and [webmockr](https://github.com/ropensci/webmockr) are recent rOpenSci packages that have mainly been used by a handful of developers until now, one of them being rOpenSci co-founder Scott Chamberlain. Scott often uses crul in his API packages while we used httr for rromeo. We were able to identify issues with vcr and webmockr when used with httr but they were fixed promptly [^1][^2][^3].
