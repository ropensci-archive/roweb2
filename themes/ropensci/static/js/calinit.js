...
var formatGoogleCalendar = (function () {
  ...
  var init = function (settings) {
    ...
    jQuery.getJSON(finalURL, function (data) {
      ...
      for (i in upcomingResult) {
        ...
        var attendees = upcomingResult[i].attendees
        for (i in attendees) {
          if (i < attendees.length - 1) {
            name = attendees[i].displayName + ', '
  ...
  var transformationList = function (result, tagName, format) {
    ...
    for (i = 0; i < format.length; i++) {
      ...
      if (format[i] === '*summary*') {
        output = output.concat('<td class="summary">' + summary + '</td>')
      } else if (format[i] === '*date*') {
        output = output.concat('<td class="date">' + dateFormatted + '</td>')
      } else if (format[i] === '*description*') {
        output = output.concat('<td class="description">' + description + '</td>')
      } else if (format[i] === '*location*') {
        output = output.concat('<td class="location">' + location + '</td>')
      } else if (format[i] === '*attendees*') {
        output = output.concat('<td class="attendees">' + name + '</td>')
      } else {
        if ((format[i + 1] === '*location*' && location !== '') ||
                    (format[i + 1] === '*summary*' && summary !== '') ||
                    (format[i + 1] === '*date*' && dateFormatted !== '') ||
                    (format[i + 1] === '*description*' && description !== '') ||
                    (format[i + 1] === '*attendees*' && name !== '')) {
